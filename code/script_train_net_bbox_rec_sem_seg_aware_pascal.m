function finetune_object_det_with_segmentation_features(model_dir_name, varargin)
...
% ip.addParamValue('selected_scales', [480, 576, 688, 874, 1200, 1600, 2100], @isnumeric);
...
%     image_db_train = load_feature_paths(image_db_train, opts.feat_cache_names);
    ...
    image_db_val = load_feature_paths(image_db_val, opts.feat_cache_names);
end
...
if opts.save_mat_model_only
    ...
    finetuned_model_path = train_net_for_object_detection(...
...
function save_to_file(filename, image_paths, feature_paths, all_proposals, all_bbox_gt)
save(filename, 'image_paths', 'feature_paths', 'all_proposals', 'all_bbox_gt', '-v7.3')
end
